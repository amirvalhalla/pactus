...
func TestOneThirdPower(t *testing.T) {
	...
	_, err := vs.AddVote(v1)
	assert.NoError(t, err)
	assert.False(t, vs.HasFPlusOneVotes(0))
	...
	_, err = vs.AddVote(v2)
	assert.NoError(t, err)
	assert.True(t, vs.HasFPlusOneVotes(0))
	...
	assert.False(t, vs.HasAnyVoteFor(0, vote.CPValueZero))
	assert.True(t, vs.HasAnyVoteFor(0, vote.CPValueOne))
	assert.False(t, vs.HasTwoFPlusOneVotesFor(0, vote.CPValueZero))
	assert.True(t, vs.HasTwoFPlusOneVotesFor(0, vote.CPValueOne))
